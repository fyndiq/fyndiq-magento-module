...
class FyndiqRegionHelper
{
    ...
    const DEFAULT_REGION_CODE_DE = 'BER';

    private static $codeNameDe = array(
        'NDS' => 'Niedersachsen',
    ...
    public static function codeToRegionCodeDe($code)
    {
        $code = intval($code);
        foreach (self::$rangesDe as $range) {
            if ($code >= $range[0] && $code <= $range[1]) {
                return $range[2];
            }
        }
        return self::DEFAULT_REGION_CODE_DE;
    }

    public static function getRegionName($code)
    {
        if (isset(self::$codeNameDe[$code])) {
            return self::$codeNameDe[$code];
        }
        throw new Exception(sprintf('Region %s not found', $code));
    }
}
