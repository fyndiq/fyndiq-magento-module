...
class Fyndiq_Fyndiq_Block_Warning extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        $label = sprintf(
            Mage::helper('fyndiq_fyndiq')->__('Please note that you are right now working on settings for the `%s` scope. Please check the selection in the top left corner of this screen and make sure you are setting up the Fyndiq Magento Extension for the correct scope depending on your requirements. Have a look at the User Guide for more information.'),
            $currentScope
        );
        $html = '<div>'.$label.'</div>';
        return $html;
